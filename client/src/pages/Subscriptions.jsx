...
const defaultForm = { name: "", amount: "", currency: "INR", category: "Other", billingCycle: "monthly", renewalDate: "", status: "active" };
...
export default function Subscriptions() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm(defaultForm); setEditId(null); setShowForm(false); fetch();
    } catch (err) { toast.error(err.response?.data?.message || "Error"); }
    finally { setSubmitting(false); }
  };

  const handleEdit = (sub) => {
    setForm({ name: sub.name, amount: sub.amount, currency: sub.currency, category: sub.category, billingCycle: sub.billingCycle, renewalDate: sub.renewalDate.split("T")[0], status: sub.status });
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.delete(`/subscriptions/${id}`); toast.success("Deleted"); fetch(); }
      ...
      <div className="max-w-screen-xl mx-auto px-5 sm:px-8 py-8">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            ...
            { label: "Yearly", value: `₹${(summary.yearlyTotal || 0).toLocaleString()}`, color: "text-violet-400", border: "border-violet-500/20", bg: "from-violet-600/10 to-transparent" },
        ...
        {showForm && (
          <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-6 mb-6">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div><label className="block text-xs font-semibold text-gray-400 uppercase tracking-widest mb-2">Amount (₹)</label><input type="number" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="649" required className={inputCls} /></div>
        ...
        {subscriptions.length > 0 ? (
          <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl overflow-hidden">
            <div className="hidden sm:grid grid-cols-[auto_1fr_120px_120px_80px_100px_60px] items-center gap-4 px-5 py-3 border-b border-white/[0.06]">
            ...
            <div className="divide-y divide-white/[0.05]">
              {subscriptions.map((sub) => (
                <div key={sub._id} className="grid grid-cols-[auto_1fr_auto] sm:grid-cols-[auto_1fr_120px_120px_80px_100px_60px] items-center gap-4 px-5 py-4 hover:bg-white/[0.02] transition group">
                  ...
                  <p className="hidden sm:block text-gray-500 text-sm">{new Date(sub.renewalDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "2-digit" })}</p>
